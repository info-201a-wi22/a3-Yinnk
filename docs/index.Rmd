...
For the five variables I calculated, I mainly focused on _comparing incarceration rates for different ethnic groups_. I calculated the **average incarceration rate for black and white** in Washington state in 2006, and the **average overall incarceration rate** across all ethnicities in the same state from 2006 to 2016. I chose to calculate these three variables because they can be used as supporting statistics for later analysis on the differences in jail admission rates in different racial groups. I also calculated **racial group with the highest population in jail** in 2016 Washington state and the **ethnic group with the highest incarceration rate**. These two variables can help to determine whether the _race with the highest jail population is the same as the race with the highest jail rate_.
...
Line_graph
```
> This line graph shows the total number of people in jail by race, from 2006 to 2016. It is very clear that the number of people in jail who are white is _much higher_ than the rest ethnicities in the dataset. The following ethinic groups with high number of jail population is black and latinx. I included this graph to show the _distribution and fluctuations of jail population for different races_. In this graph, we can tell that the jail population for white has an overall trend of decreasing over the past 10 years while the jail population for other races fluctuated slightly but didn’t have any major changes. 

```{r, echo=FALSE,warning=FALSE, message=FALSE}
source("../source/analysis.R")
comparison_graph
```
>This bar chart shows the comparison between average incarceration rates between _two different racial groups and the overall average rate_. I included this graph because the previous line graph shows that the white people occupy the highest jail population for all 10 years. However, that does not necessarily mean that they have the highest incarceration rate considering the population. The data used to calculate the incarceration rate here is collected for the year 2016. I focused on the data from this very recent yea particularly because I want to present to audiences with the updated situation of juridical treatment. People may think that the fairness of the juridical system has been improved greatly but from this graph we can tell that the _incarceration rate for black people is still significantly higher_ than the average rate for white and the average number for all races. In contrast, the rate of jail admission for white is even slightly lower than the average rate for all ethnicities. 
